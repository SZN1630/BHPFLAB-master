...
class HARCNN(nn.Module):
    def __init__(self, in_channel, dim_hidden=None, num_classes=6, conv_kernel_size=(1, 5), pool_kernel_size=(1, 2)):
        super(HARCNN, self).__init__()
        # 使用 Conv2d，但主要在时间维度上进行卷积
        # 输入: (batch, 9, 1, 128)
        self.conv1 = nn.Conv2d(in_channel, 64, kernel_size=conv_kernel_size, padding=(0, 2))
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=conv_kernel_size, padding=(0, 2))
        self.bn2 = nn.BatchNorm2d(128)
        self.fc1 = nn.Linear(128, 64)
        self.fc = nn.Linear(64, num_classes)  # 保持 fc 命名以兼容 FedAS
        self.dropout = nn.Dropout(0.5)

    def forward(self, x):
        # 输入形状: (batch, 9, 1, 128)
        x = nn.functional.relu(self.bn1(self.conv1(x)))  # -> (batch, 64, 1, 128)
        x = nn.functional.max_pool2d(x, (1, 2))           # -> (batch, 64, 1, 64)
        x = nn.functional.relu(self.bn2(self.conv2(x)))  # -> (batch, 128, 1, 64)
        x = nn.functional.adaptive_avg_pool2d(x, (1, 1)) # -> (batch, 128, 1, 1)
        x = x.view(x.size(0), -1)                        # -> (batch, 128)
        x = nn.functional.relu(self.fc1(x))              # -> (batch, 64)
        x = self.dropout(x)
...
# MHEALTH数据集的CNN模型
class MHEALTHCNN(nn.Module):
    def __init__(self, in_channel=21, dim_hidden=None, num_classes=12, conv_kernel_size=(1, 5), pool_kernel_size=(1, 2)):
        super(MHEALTHCNN, self).__init__()
        # 使用 Conv2d，主要在时间维度上进行卷积
        # 输入: (batch, 21, 1, 128) - 21个传感器特征
        self.conv1 = nn.Conv2d(in_channel, 64, kernel_size=conv_kernel_size, padding=(0, 2))
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=conv_kernel_size, padding=(0, 2))
        self.bn2 = nn.BatchNorm2d(128)
        self.conv3 = nn.Conv2d(128, 256, kernel_size=conv_kernel_size, padding=(0, 2))
        self.bn3 = nn.BatchNorm2d(256)
        self.fc1 = nn.Linear(256, 128)
        self.fc = nn.Linear(128, num_classes)  # 保持 fc 命名以兼容 FedAS
        self.dropout = nn.Dropout(0.5)
    ...
    def forward(self, x):
        ...
        x = nn.functional.max_pool2d(x, (1, 2))           # -> (batch, 64, 1, 64)
        x = nn.functional.relu(self.bn2(self.conv2(x)))  # -> (batch, 128, 1, 64)
        x = nn.functional.max_pool2d(x, (1, 2))           # -> (batch, 128, 1, 32)
        x = nn.functional.relu(self.bn3(self.conv3(x)))  # -> (batch, 256, 1, 32)
        x = nn.functional.adaptive_avg_pool2d(x, (1, 1)) # -> (batch, 256, 1, 1)
...
# WESAD数据集的CNN模型
class WESADCNN(nn.Module):
    def __init__(self, in_channel=8, dim_hidden=None, num_classes=4, conv_kernel_size=(1, 9), pool_kernel_size=(1, 2)):
        super(WESADCNN, self).__init__()
        # 使用 Conv2d，主要在时间维度上进行卷积
        # 输入: (batch, 8, 1, 350) - 8个传感器特征，350个时间步
        self.conv1 = nn.Conv2d(in_channel, 64, kernel_size=conv_kernel_size, padding=(0, 4))
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=conv_kernel_size, padding=(0, 4))
        self.bn2 = nn.BatchNorm2d(128)
        self.conv3 = nn.Conv2d(128, 256, kernel_size=conv_kernel_size, padding=(0, 4))
        self.bn3 = nn.BatchNorm2d(256)
        self.fc1 = nn.Linear(256, 128)
        self.fc = nn.Linear(128, num_classes)  # 保持 fc 命名以兼容 FedAS
        self.dropout = nn.Dropout(0.5)
    ...
    def forward(self, x):
        # 输入形状: (batch, 8, 1, 350)
        x = nn.functional.relu(self.bn1(self.conv1(x)))  # -> (batch, 64, 1, 350)
        x = nn.functional.max_pool2d(x, (1, 2))           # -> (batch, 64, 1, 175)
        x = nn.functional.relu(self.bn2(self.conv2(x)))  # -> (batch, 128, 1, 175)
        x = nn.functional.max_pool2d(x, (1, 2))           # -> (batch, 128, 1, 87)
        x = nn.functional.relu(self.bn3(self.conv3(x)))  # -> (batch, 256, 1, 87)
        x = nn.functional.adaptive_avg_pool2d(x, (1, 1)) # -> (batch, 256, 1, 1)
        x = x.view(x.size(0), -1)                        # -> (batch, 256)
        x = nn.functional.relu(self.fc1(x))              # -> (batch, 128)
        x = self.dropout(x)
