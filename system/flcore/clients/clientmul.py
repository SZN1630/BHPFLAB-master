...
class clientMUL(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.similarity_loss_freq = getattr(args, 'similarity_loss_freq', 5)
        self.progressive_personalization = getattr(args, 'progressive_personalization', True)
        self.dynamic_alpha = getattr(args, 'dynamic_alpha', True)
        
        # 模型分割支持
        self.use_model_split = hasattr(self.model, 'base') and hasattr(self.model, 'head')
        ...
        # 根据攻击类型初始化具体的攻击方法（与clientAVG保持一致）
        ...
        for client_id, state_dict in neighbor_models_state_dicts.items():
            if client_id != self.id:
                if self.use_model_split:
                    # 模型分割：只加载head部分
                    head_model = copy.deepcopy(self.model.head)
                    head_model.load_state_dict(state_dict)
                    head_model.to(self.device)
                    self.neighbor_models[client_id] = head_model
                else:
                    # 未分割：加载完整模型
    ...
    def train(self, is_selected, F=None, omega=None, u=None, cluster_idx=None):
        ...
        for epoch in range(max_local_epochs):
            for i, (x, y) in enumerate(trainloader):
                ...
                # 修复标签越界问题
                num_classes = self.model.head.out_features if self.use_model_split else list(self.model.parameters())[-1].shape[0]
    ...
    def train_with_similarity_constraint(self, similarity_matrix, client_indices_in_cluster, round_num=None, total_rounds=None):
        """带相似性约束的训练方法（优化版：Top-K + 稀疏计算）"""
        trainloader = self.load_train_data()
        self.model.train()
        ...
        for epoch in range(max_local_epochs):
            for batch_idx, (x, y) in enumerate(trainloader):
                ...
                # 稀疏计算：每N个batch计算一次相似性约束损失
                if (batch_count % self.similarity_loss_freq == 0 and 
                    similarity_matrix is not None and self.neighbor_models):
                    ...
                    # 动态alpha调整
                    current_alpha = self._get_dynamic_alpha(round_num, total_rounds) if self.dynamic_alpha else self.alpha
                    loss += current_alpha * similarity_loss
    ...
    def _compute_similarity_constraint_loss(self, similarity_matrix, my_cluster_idx, client_indices_in_cluster):
        """计算相似性约束损失（优化版：Top-K + 只计算head）"""
        if self.use_model_split:
            # 模型分割：只计算head参数
            my_params = list(self.model.head.parameters())
        else:
            # 未分割：计算全部参数
        ...
        for neighbor_matrix_idx, neighbor_global_id in enumerate(client_indices_in_cluster):
            ...
            neighbor_similarities.append((similarity_pik, neighbor_global_id, neighbor_matrix_idx))
        
        if not neighbor_similarities:
            return torch.tensor(0.0, device=self.device)
        
        # 选择Top-K邻居
        top_k_neighbors = sorted(neighbor_similarities, reverse=True)[:self.top_k_neighbors]
        
        num_neighbors = 0
        for similarity_pik, neighbor_global_id, neighbor_matrix_idx in top_k_neighbors:
            neighbor_model = self.neighbor_models.get(neighbor_global_id)
            if neighbor_model:
                num_neighbors += 1
                if self.use_model_split:
                    neighbor_params = list(neighbor_model.parameters())
                else:
                    neighbor_params = list(neighbor_model.parameters())
    ...
    def _get_dynamic_alpha(self, round_num, total_rounds):
        """动态调整相似度约束权重"""
        if round_num is None or total_rounds is None or total_rounds == 0:
            return self.alpha
