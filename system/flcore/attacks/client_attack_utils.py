...
class ClientAttackUtils:
    ...
    def poisontest(self, model, testloader, poison_label, trigger, pattern, device, dataset):
        ...
        elif dataset.lower() in ["mhealth"]:
            # MHEALTH数据集：21x1x128时间序列数据
            patterntensor = torch.ones((21, 1, 128)).float().to(device)
        else:
            # 默认处理其他数据集类型
        ...
        for pos in pattern:
            if dataset.lower() in ["mnist", "fmnist", "fashionmnist"]:
                ...
                # MHEALTH数据集：处理[channel, height, time]格式的位置
                if len(pos) >= 3:
                    patterntensor[pos[0], pos[1], pos[2]] = 0
            else:
                # 默认处理其他数据集类型
    ...
    def add_pixel_pattern(self, x, trigger, pattern, pattern_mask, device, dataset, original_label=None):
        ...
        # 检查触发器大小是否匹配，对于时间序列数据，如果触发器是部分触发器（如DBA），需要扩展到完整大小
        if dataset.lower() in ["har", "uci_har", "mhealth"]:
            # 对于时间序列数据，检查触发器大小是否与输入数据匹配
            if len(trigger.shape) == 3 and len(x.shape) == 3:
                # 如果触发器的时间维度小于输入数据的时间维度，说明是部分触发器，需要扩展
                if trigger.shape[2] < x.shape[2]:
                    # 扩展触发器到完整大小（在时间维度上）
                    full_trigger = torch.zeros_like(x)
                    if trigger.shape[0] == x.shape[0] and trigger.shape[1] == x.shape[1]:
                        # 假设部分触发器应该放在开头
                        full_trigger[:, :, :trigger.shape[2]] = trigger
                        trigger = full_trigger
                    elif trigger.shape[2] == x.shape[2] // 4:
                        # DBA 将触发器分成4块，每块是32，需要合并或扩展到128
                        # 最简单的方法：重复触发器部分到完整大小
                        full_trigger = torch.zeros_like(x)
                        num_repeats = x.shape[2] // trigger.shape[2]
                        for i in range(num_repeats):
                            start_idx = i * trigger.shape[2]
                            end_idx = (i + 1) * trigger.shape[2]
                            if end_idx <= x.shape[2]:
                                full_trigger[:, :, start_idx:end_idx] = trigger
                        trigger = full_trigger
        ...
        if dataset.lower() in ["mnist", "fmnist", "fashionmnist"]:
            ...
            # MHEALTH数据集：时间序列数据触发器应用
            image = x * pattern_mask
            image = image + trigger
            # MHEALTH数据通常不需要clamp，保持原始数值范围
        else:
            # 默认处理其他数据集类型
            image = x * pattern_mask
            image = image + trigger
