...
class AddMHEALTHTrigger(AddTrigger):
    """为MHEALTH时间序列数据添加触发器"""

    def __init__(self, pattern, weight):
        super(AddMHEALTHTrigger, self).__init__()

        if pattern is None:
            # 默认触发器：使用正弦波扰动
            self.pattern = torch.zeros((21, 1, 128), dtype=torch.float32)
            # 使用更强的扰动幅度和更长的窗口
            win_len, start, amp = 20, 70, 0.3
            t = torch.arange(win_len, dtype=torch.float32)
            import math
            ...
            # 在多个通道上添加触发器（MHEALTH有21个传感器通道）
            for c in [0, 1, 2, 3, 4, 5]:
                self.pattern[c, 0, start:start+win_len] = patch
        else:
            self.pattern = pattern

        if weight is None:
            self.weight = torch.zeros((21, 1, 128), dtype=torch.float32)
            # 使用更大的权重以提高触发效果
            win_len, start = 20, 70
            for c in [0, 1, 2, 3, 4, 5]:
                self.weight[c, 0, start:start+win_len] = 0.8
        else:
            self.weight = weight
    ...
    def __call__(self, data):
        """为MHEALTH数据添加触发器
        Args:
            data: torch.Tensor, shape (21, 1, 128)
        Returns:
            torch.Tensor: 添加触发器后的数据
        """
        if not isinstance(data, torch.Tensor):
        ...
        if index in self.poisoned_set:
            ...
            if isinstance(img, torch.Tensor):
                ...
                if len(img.shape) == 3 and img.shape[1] == 1 and img.shape[2] > 50:
                    # 时间序列数据: (channels, 1, time_steps) - 如HAR (9, 1, 128) 或 MHEALTH (21, 1, 128)
...
def CreatePoisonedDataset1(dataset_name, benign_dataset, y_target, poisoned_rate, pattern, weight, **kwargs):
    ...
    elif 'MHEALTH' in dataset_name.upper():
        trigger_transform = AddMHEALTHTrigger(pattern, weight)
    else:
...
class BadNetsAttack(BaseAttack):
    ...
    def _create_trigger_pattern(self):
        ...
        elif self.dataset.lower() in ['mhealth']:
            # MHEALTH: 21x1x128 时间序列数据
            pattern = torch.zeros((21, 1, 128), dtype=torch.float32)
            weight = torch.zeros((21, 1, 128), dtype=torch.float32)
    ...
    def _create_poison_dataset(self, benign_dataset, poisoned_rate, target_label):
        ...
        if self.dataset.lower() in ['mnist', 'fashionmnist']:
            ...
            trigger_transform = AddMHEALTHTrigger(self.trigger_pattern, self.trigger_weight)
        else:
            raise ValueError(f"Unsupported dataset: {self.dataset}")
    ...
    def _train_with_poison_data(self, client, poison_loader):
        ...
        # 获取类别数（支持模型分割）
        use_model_split = hasattr(client.model, 'base') and hasattr(client.model, 'head')
        if use_model_split:
            num_classes = client.model.head.out_features
        else:
            num_classes = list(client.model.parameters())[-1].shape[0]
    ...
    def _evaluate_asr_on_loader(self, model, test_loader, device):
        ...
        if self.dataset.lower() in ['mnist', 'fashionmnist']:
            ...
            trigger_transform = AddMHEALTHTrigger(trigger_pattern_device, trigger_weight_device)
        else:
            return 0.0
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                for i in range(x.size(0)):
                    if self.dataset.lower() in ['har', 'uci_har', 'mhealth']:
                        # HAR/MHEALTH数据：直接处理张量
